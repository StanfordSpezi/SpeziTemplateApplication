...
class OnboardingTests: XCTestCase {
    ...
    private static func navigateOnboardingFlow() throws {
        ...
        XCTAssertTrue(app.staticTexts["This application demonstrates several CardinalKit features & modules."].waitForExistence(timeout: 0.5))
        
        XCTAssertTrue(app.staticTexts["The CardinalKit Framework"].waitForExistence(timeout: 0.5))
        XCTAssertTrue(app.staticTexts["The CardinalKit Framework builds the foundation of this template application."].waitForExistence(timeout: 0.5))
        
        XCTAssertTrue(app.staticTexts["Swift Package Manager"].waitForExistence(timeout: 0.5))
        XCTAssertTrue(app.staticTexts["CardinalKit uses the Swift Package Manager to import it as a dependency."].waitForExistence(timeout: 0.5))
        
        XCTAssertTrue(app.staticTexts["CardinalKit Modules"].waitForExistence(timeout: 0.5))
        XCTAssertTrue(app.staticTexts["CardinalKit offers several modules including HealthKit integration, questionnaires, and more ..."].waitForExistence(timeout: 0.5))
        ...
        XCTAssertTrue(app.staticTexts["Here are a few CardinalKit modules that are interesting to know about ..."].waitForExistence(timeout: 0.5))
        
        XCTAssertTrue(app.staticTexts["Onboarding"].waitForExistence(timeout: 0.5))
        XCTAssertTrue(app.staticTexts["The onboarding module allows you to build an onboarding flow like this one."].waitForExistence(timeout: 0.5))
        ...
        XCTAssertTrue(app.staticTexts["The FHIR module provides a CardinalKit standard that can be used as a communication standard between modules."].waitForExistence(timeout: 0.5))
        XCTAssertTrue(app.buttons["Next"].waitForExistence(timeout: 0.5))
        app.buttons["Next"].tap()
        
        XCTAssertTrue(app.staticTexts["Contact"].waitForExistence(timeout: 0.5))
        XCTAssertTrue(app.staticTexts["The contact module allows you to display contact information in your application."].waitForExistence(timeout: 0.5))
        XCTAssertTrue(app.buttons["Next"].waitForExistence(timeout: 0.5))
        app.buttons["Next"].tap()
        
        XCTAssertTrue(app.staticTexts["HealthKit Data Source"].waitForExistence(timeout: 0.5))
        XCTAssertTrue(app.staticTexts["The HealthKit data source module allows you to fetch data from HealthKit and e.g. transform it to FHIR resources."].waitForExistence(timeout: 0.5))
        XCTAssertTrue(app.buttons["Next"].waitForExistence(timeout: 0.5))
        app.buttons["Next"].tap()
        
        // Consent View
        XCTAssertTrue(app.staticTexts["Consent Example"].waitForExistence(timeout: 0.5))
        XCTAssertTrue(app.staticTexts["CardinalKit can collect consent from a user. You can provide the consent document using a markdown file."].waitForExistence(timeout: 0.5))
        XCTAssertTrue(app.staticTexts["CardinalKit can render consent documents in the markdown format: This is a markdown example.\n"].waitForExistence(timeout: 0.5))
        ...
        XCTAssertTrue(app.staticTexts["CardinalKit can access data from HealthKit using the HealthKitDataSource module."].waitForExistence(timeout: 0.5))
